...
//
//   node crear-manuales-agotados.mjs           -> simulacion
//   node crear-manuales-agotados.mjs --aplicar -> escribe de verdad
// ----------------------------------------------------------------------
...
// --- Lo que se crea ---------------------------------------------------------
// Los cuatro manuales de las divisiones. RD$500 para registrados y no
// registrados, porque no se dio otro precio. "Libro" delante del nombre, como el
// de Douglas Marsh: en la rejilla distingue un libro de la carpeta o del parche
...
const TODOS = [
  manual('Libro Manual del Pionero de los Exploradores del Rey', 'pionero.webp'),
  manual('Libro Manual del Seguidor de la Senda de los Exploradores del Rey', 'seguidor-senda.webp'),
  manual('Libro Manual del Explorador de Aire-Mar-Senda de los Exploradores del Rey', 'aire-mar-senda.webp'),
  manual('Libro Manual del Líder de los Exploradores del Rey', 'lider.webp'),
];

TODOS.forEach((p) => {
  if (!fs.existsSync(p.imagen)) throw new Error(`No existe la imagen ${p.imagen}`);
});
...
const construirProducto = ({ productoId, codigo, p, imagenes }) => {
  ...
  return {
    ...
    renglon: p.renglon,
    requiereAprobacion: p.renglon === 'restringido',
    tipoProducto: 'simple',
    variantes: [],
    ...
    etiquetas: [],
    genero: [],
    etiquetaNuevo: { habilitada: Boolean(p.cinta), contenido: p.cinta || '' },
  ...
  for (const p of TODOS) {
    ...
    if (APLICAR) {
      ...
      await auditRef.set({
        ...
        origen: 'script',
        metadatos: { ambito: 'tienda', lote: 'alta-manuales-agotados' },
    ...
    console.log(
      `  ${codigo}  ${p.nombre}${p.imagen ? '  [con imagen]' : ''}${p.cinta ? `  [cinta: ${p.cinta}]` : ''}  RD$${p.registrado}/${p.noRegistrado}  x${p.cantidad}`
    );
