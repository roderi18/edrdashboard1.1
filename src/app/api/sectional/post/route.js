...
export async function POST(req) {
  try {
    // Sin sesion no se crea una seccion.
    ...
    const res = await fetch('https://systexploradores.somee.com/api/Secciones/SetSecciones', {
      method: 'POST',
      headers: {
    ...
    try {
      ...
      return Response.json({ error: 'Respuesta no es JSON', raw: text }, { status: 500 });
    }

    return Response.json(normalizeApiResponse(data));
  } catch (error) {
    return Response.json({ error: 'Error creando sección' }, { status: 500 });
  }
}
