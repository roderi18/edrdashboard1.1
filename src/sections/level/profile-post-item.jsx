...
import { useMockedLevel } from 'src/auth/hooks';

// ----------------------------------------------------------------------

export function ProfilePostItem({ post }) {
  const { level } = useMockedLevel();
  ...
  const renderHead = () => (
    <CardHeader
      disableTypography
      avatar={
        <Avatar src={level?.photoURL} alt={level?.displayName}>
          {level?.displayName?.charAt(0).toUpperCase()}
        </Avatar>
      }
      title={
