...
export function SectionalLeadershipView({
  ...
  const [vistaSeccional, setVistaSeccional] = useState('directiva');
  ...
  return (
    <>
      {!embebido && (
        <Tabs
          value={vistaSeccional}
          onChange={(event, valor) => setVistaSeccional(valor)}
          sx={{ mb: 2 }}
        >
      ...
      {!embebido && vistaSeccional === 'historia' && (
        <LeadershipHistoryTab nivel="seccional" idEntidad={sectionalId} />
      )}

      {(embebido || vistaSeccional === 'directiva') && (
      <Box
        ref={containerRef}
