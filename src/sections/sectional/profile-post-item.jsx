...
import { useMockedSectional } from 'src/auth/hooks';

// ----------------------------------------------------------------------

export function ProfilePostItem({ post }) {
  const { sectional } = useMockedSectional();
  ...
  const renderHead = () => (
    <CardHeader
      disableTypography
      avatar={
        <Avatar src={sectional?.photoURL} alt={sectional?.displayName}>
          {sectional?.displayName?.charAt(0).toUpperCase()}
        </Avatar>
      }
      title={
        <Link color="inherit" variant="subtitle1">
          {sectional?.displayName}
        </Link>
      }
      subheader={
