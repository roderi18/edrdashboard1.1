...
export function AwardsManagerView({ memberId, readOnly = false }) {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined' && memberId) {
            const stored = JSON.parse(
                localStorage.getItem(`awards-status-${memberId}`) || '{}'
            );
    ...
    const sistemaAscensoIndex = folderBreadcrumbs.findIndex(
        (b) => b.name === 'Sistema de Ascenso'
    );
    ...
    const isAcademiaSubFolder =
        ...
        tableData.some(
            (item) =>
                item.id === normalizedFolder &&
                item.parentId === ACADEMIA_MINISTERIAL_ID
        );

    const isSistemaAscenso =
        normalizedFolder === SISTEMA_ASCENSO_ID;
    const isSistemaAscensoRootFolder = normalizedFolder === SISTEMA_ASCENSO_ID;

    const isSistemaAscensoSubFolder =
        sistemaAscensoIndex !== -1 &&
        folderBreadcrumbs.length === sistemaAscensoIndex + 2;

    const isSistemaAscensoDeepSubFolder =
        sistemaAscensoIndex !== -1 &&
        folderBreadcrumbs.length >= sistemaAscensoIndex + 3;
    ...
    const dataByFolder = tableData.filter((item) =>
        ...
        normalizedFolder
            ? item.parentId === normalizedFolder
            : item.parentId == null
    );
    ...
    const dataWithStatus = dataByFolder.map((item) => {
        ...
        if (isSistemaAscensoDeepSubFolder) {
            ...
            realStatus =
                statusStorage?.[systemKey]?.[normalizedFolder]?.[item.id] ?? null;
        }
