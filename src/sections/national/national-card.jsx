...
export function NationalCard({ national, sx, ...other }) {
  return (
    <Card sx={[{ textAlign: 'center' }, ...(Array.isArray(sx) ? sx : [sx])]} {...other}>
      ...
      <ListItemText
        sx={{ mt: 7, mb: 1 }}
        primary={national.name}
        secondary={national.role}
        slotProps={{
      ...
      >
        {[
          { label: 'Follower', value: national.totalFollowers },
          { label: 'Following', value: national.totalFollowing },
          { label: 'Total post', value: national.totalPosts },
        ].map((stat) => (
