...
import { useMockedNational } from 'src/auth/hooks';

// ----------------------------------------------------------------------

export function ProfilePostItem({ post }) {
  const { national } = useMockedNational();
  ...
  const renderHead = () => (
    <CardHeader
      disableTypography
      avatar={
        <Avatar src={national?.photoURL} alt={national?.displayName}>
          {national?.displayName?.charAt(0).toUpperCase()}
        </Avatar>
      }
      title={
