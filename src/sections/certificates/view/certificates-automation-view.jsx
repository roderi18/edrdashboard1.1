...
export const getTemplatePdfTypography = (field) => {
  const fontFamily = normalizeTemplateFont(field.fontFamily);
...
export const DEFAULT_TEMPLATE_POSITIONS = {
...
export const isQrTemplateField = (field) => field?.kind === 'qr' || field?.id === 'qrCode';

export const getTemplateFieldSize = (field) => Number(field?.size || field?.width || 72);
...
const generarPdfDeCertificados = async (props) => {
  const { generarPdfDeCertificadosDocumento } = await import('./certificate-pdf-documento');
  ...
  const buildCertificatesForSelectedMembers = async () => {
    ...
    const certificateFiles = await Promise.all(
      selectedMembers.map(async (member) => ({
        ...
        blob: await generarPdfDeCertificados({
          course: selectedCourse,
          members: [member],
    ...
    const combinedBlob = await generarPdfDeCertificados({
      course: selectedCourse,
