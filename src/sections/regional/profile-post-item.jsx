...
import { useMockedRegional } from 'src/auth/hooks';

// ----------------------------------------------------------------------

export function ProfilePostItem({ post }) {
  const { regional } = useMockedRegional();
  ...
  const renderHead = () => (
    <CardHeader
      disableTypography
      avatar={
        <Avatar src={regional?.photoURL} alt={regional?.displayName}>
          {regional?.displayName?.charAt(0).toUpperCase()}
        </Avatar>
      }
      title={
